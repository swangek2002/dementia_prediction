...
echo "===== no-SAW — Test-Only Evaluation (Single GPU) ====="
echo "Start: $(date)"
echo ""
echo "  Using best checkpoint: crPreTrain_small_1337_FineTune_Dementia_CR_noSAW.ckpt"
echo "  train=False, test=True, devices=1"
echo "  log_individual=True → dementia CIF + death CIF metrics separately"
echo ""

cd "$WORK_DIR"
$PYTHON run_experiment.py --config-name=config_FineTune_Dementia_CR_noSAW_eval

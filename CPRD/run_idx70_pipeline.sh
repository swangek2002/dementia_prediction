...
LOG_FILE="${LOG_DIR}/finetune_cr_idx70_log.txt"

cd "$WORK_DIR"

########################################
# Step 1: Build dataset (index_age=70)
########################################
echo "===== Step 1: Build dataset (index_age=70, 31 pure codes) =====" | tee "$LOG_FILE"
echo "Start: $(date)" | tee -a "$LOG_FILE"

$PYTHON build_dementia_cr_idx70_dataset.py 2>&1 | tee -a "$LOG_FILE"
...
echo "===== Step 2: Train (event_only, lambda=5, idx70, 15 epochs) =====" | tee -a "$LOG_FILE"
echo "Start train: $(date)" | tee -a "$LOG_FILE"

rm -f "${CKPT_DIR}/crPreTrain_small_1337_FineTune_Dementia_CR_idx70.ckpt" 2>/dev/null || true

$PYTHON run_experiment.py --config-name=config_FineTune_Dementia_CR_idx70 2>&1 | tee -a "$LOG_FILE"
...
CUDA_VISIBLE_DEVICES=0 $PYTHON run_experiment.py --config-name=config_FineTune_Dementia_CR_idx70_eval 2>&1 | tee -a "$LOG_FILE"

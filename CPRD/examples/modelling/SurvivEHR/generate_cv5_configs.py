...
Output:
    confs/config_FineTune_Dementia_CR_idx68_cv_fold{0..4}.yaml
    confs/config_FineTune_Dementia_CR_idx68_cv_fold{0..4}_eval.yaml
"""
...
def make_config(fold: int, is_eval: bool) -> str:
    ds_path = f"/Data0/swangek_data/991/CPRD/data/FoundationalModel/FineTune_Dementia_CR_idx68_cv/fold{fold}/"
    fine_tune_id = f"FineTune_Dementia_CR_idx68_cv_fold{fold}"
    ...
    notes = f"{'Single-GPU eval' if is_eval else 'Train'} — idx68 5-fold CV fold {fold}, event_only lambda=6"
...
experiment:
  ...
  notes: "{notes}"
  tags: ["dementia", "fine-tune", "competing-risk", "idx68", "cv5", "fold{fold}", "{mode_label}"]
  train: {train_flag}
  test: True
  verbose: True
  seed: 1337
...
def main():
    ...
    for fold in range(5):
        ...
        train_path = confs_dir / f"config_FineTune_Dementia_CR_idx68_cv_fold{fold}.yaml"
        train_path.write_text(train_cfg)
        print(f"Written: {train_path}")

        # Eval config
        eval_cfg = make_config(fold, is_eval=True)
        eval_path = confs_dir / f"config_FineTune_Dementia_CR_idx68_cv_fold{fold}_eval.yaml"

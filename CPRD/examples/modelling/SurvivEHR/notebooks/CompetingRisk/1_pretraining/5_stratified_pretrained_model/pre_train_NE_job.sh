...
#SBATCH --job-name=pretrain_NE_run
#SBATCH --output=out/NE-job-%j.out
...
# 
echo "Training SurvivEHR Foundation Model on patients from North East of England"
...
python run_experiment.py  --config-name=config_CompetingRisk11M \
    experiment.project_name="SurvivEHR-NE-OOD" \
    experiment.run_id="SurvivEHR-NorthEast-v1-v1-v1" \
    ...
    optim.scheduler_periods=1000  \
    optim.learning_rate=1e-4 \
    optim.limit_test_batches=0.05
    
    # 3e-4, 1.5e-4, 1e-4, 5e-5
    

# data.path_to_ds="/rds/projects/g/gokhalkm-optimal/OPTIMAL_MASTER_DATASET/data/FoundationalModel/ByRegion/PreTrain_North East/" \
